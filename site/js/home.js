...
window.addEvent( 'domready' , function(event){
	...
	$$('.project-container')[0].getElements('.block > .bollonzo').setStyle( 'top' , -650 );


	$$('.project-container')[0].scrollTo( window.getSize().x * 2 , 0 );

 	new Element("script", { "type": "text/javascript", "async": true, "src": "http://twitter.com/status/user_timeline/simedia.json?count=10&callback=twitterCallBack" }).inject($(document.head), "bottom");
...
function deActivateProject( project , callback , outTime ){
	...
	if( project.retrieve( 'indexProject' ) == totalProjects ){
		...
		( function(){ project.getElements('.block > .bollonzo').setStyle( 'top' , 0 ); } ).delay( timeOfActivate * 2 );
...
function activateProject( project , callback ){
	...
	if( animationIn ) eval(animationIn)(); else enterBlockDefault();
	...
	( function(){
 		...
 		project.getElements('.block > .bollonzo').tween( 'top' , -650 );
